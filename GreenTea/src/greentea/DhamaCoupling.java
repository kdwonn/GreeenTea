...
public class DhamaCoupling {
	...
	private int CalcAfferentCoupling(IMethod targetMethod, IPackageFragment currentPack) {
		...
		try {
			...
			IJavaSearchScope scope = 
					SearchEngine.createJavaSearchScope(MartinCoupling.getOuterPackages(currentPack)
								.toArray(new IJavaElement[] {}));
			SearchPattern pattern = 
					SearchPattern.createPattern(targetMethod, IJavaSearchConstants.REFERENCES);
			AfferentRequestor requestor = 
					new AfferentRequestor(currentPack);
	...
	private int getPublicFieldRef(IMethod targetMethod, List<FieldDeclaration> fieldList) {
		int result = 0;
		if(fieldList.size() == 0) return 0;
		try {
			SearchEngine searchEngine = new SearchEngine();
			IJavaSearchScope scope = SearchEngine.createJavaSearchScope(new IJavaElement[] {targetMethod});
			SearchPattern pattern = SearchPattern.createPattern((IJavaElement)fieldList.get(0).fragments().get(0), IJavaSearchConstants.REFERENCES);
			for(int i = 1; i < fieldList.size(); i ++) {
				SearchPattern addedP = SearchPattern.createPattern((IJavaElement)fieldList.get(i).fragments().get(0), IJavaSearchConstants.REFERENCES);

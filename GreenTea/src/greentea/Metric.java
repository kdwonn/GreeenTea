...
public class Metric {
	...
	static public double measureMaintain(IMethod method, String projectName, String packageName, String className,
			String methodName) {
		double result = Math.max(0,
				(171 - 5.2 * Math.log(Metric.measureHalstead(projectName, packageName, className, methodName))
						- 0.23 * Metric.measureCyclomatic(projectName, packageName, className, methodName)
						- 16.2 * Math.log(Metric.measureLOC(method))) * 100 / 171);
		return Math.round(result * 100d) / 100d;
	}

	static public double measureMaintainWithSource(String source, String methodName) {
		double result = Math.max(0,
				(171 - 5.2 * Math.log(Metric.measureHalsteadWithSource(source, methodName))
						- 0.23 * Metric.measureCyclomaticWithSource(source, methodName)
						- 16.2 * Math.log(Metric.measureLOC(source))) * 100 / 171);
		return Math.round(result * 100d) / 100d;
	}
